...
mod adversarial_probe {
    ...
    #[test]
    fn probe_kill_arm_then_resort_moves_target() {
        let mut s = st();
        s.handle_status(&args(&[("pane_id", "5"), ("status", "working")]));
        s.handle_status(&args(&[("pane_id", "9"), ("status", "working")]));
        s.selected = 0;
        let armed_target = s.agents[0].id.clone();
        s.handle_key(KeyWithModifier::new(BareKey::Char('x')));
        assert_eq!(s.kill_armed, Some(armed_target.clone()));

        // An unrelated pane fails -> re-sorts to the top, shifting index 0.
        s.handle_status(&args(&[("pane_id", "9"), ("status", "failed")]));
        eprintln!(
            "PROBE after resort: selected={} rows={:?} armed={:?}",
            s.selected,
            s.agents.iter().map(|a| (a.id.pane_id, a.status)).collect::<Vec<_>>(),
            s.kill_armed.as_ref().map(|i| i.pane_id)
        );

        s.handle_key(KeyWithModifier::new(BareKey::Char('x')));
        eprintln!(
            "PROBE after 2nd x: rows={:?} armed={:?}",
            s.agents.iter().map(|a| (a.id.pane_id, a.status)).collect::<Vec<_>>(),
            s.kill_armed.as_ref().map(|i| i.pane_id)
        );
    }
    ...
    #[test]
    fn probe_reply_target_vs_editor_target() {
        let mut s = st();
        s.handle_status(&args(&[("pane_id", "5"), ("status", "waiting")]));
        s.handle_status(&args(&[("pane_id", "9"), ("status", "working")]));
        s.selected = s.agents.iter().position(|a| a.id.pane_id == 5).unwrap();
        assert!(s.begin_reply());
        let composing_for = s.reply.as_ref().unwrap().id.pane_id;
        // pane 9 fails: rank 0, sorts above the waiting row.
        s.handle_status(&args(&[("pane_id", "9"), ("status", "failed")]));
        eprintln!(
            "PROBE reply rows={:?} selected={} composing_for={}",
            s.agents.iter().map(|a| (a.id.pane_id, a.status)).collect::<Vec<_>>(),
            s.selected,
            composing_for
        );
        ...
        eprintln!(
            "PROBE after enter rows={:?}",
            s.agents
                .iter()
                .map(|a| (a.id.pane_id, a.status, a.detail.clone()))
                .collect::<Vec<_>>()
        );
    }

    #[test]
    fn probe_summary_gating_when_path_empty() {
        let mut s = st();
        s.summary_path = String::new();
        s.handle_status(&args(&[("pane_id", "1"), ("status", "waiting")]));
        eprintln!("PROBE last_summary={:?}", s.last_summary);
        s.summary_path = "/tmp/x".into();
        s.publish_summary();
        eprintln!("PROBE after path set last_summary={:?}", s.last_summary);
    }
}
